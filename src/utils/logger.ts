...
export class Logger {
  ...
  private initializeLogFile(): void {
    ...
    try {
      ...
      if (fs.existsSync(this.config.file)) {
        ...
        if (
          this.currentFileSize >= (this.config.maxFileSize || 10 * 1024 * 1024)
        ) {
          this.rotateLogFile();
        }
      }
  ...
  private writeLog(entry: LogEntry): void {
    ...
    if (this.config.enableConsole && !this.config.quiet) {
      ...
      // Always write to stderr so diagnostic logs don't pollute stdout
      // (important when stdout is piped, e.g. `cdp screenshot > page.png`)
  ...
  warn(
    message: string,
    data?: any,
    context?: { component?: string; connectionId?: string; clientId?: string },
  ): void {
