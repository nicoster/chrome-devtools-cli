...
export class HelpSystem {
  ...
  generateCommandHelp(commandName: string): string {
    ...
    if (command.options.length > 0) {
      ...
      for (const option of command.options) {
        ...
        const negation =
          option.type === "boolean" ? `, --no-${option.name}` : "";
  ...
  generateGeneralHelp(): string {
    ...
    const orderedCategories: Array<[string, CommandDefinition[]]> = [];
  ...
  private generateTopicNotFoundHelp(topicName: string): string {
    ...
    const topics = Array.from(this.helpTopics.values()).sort((a, b) =>
      a.name.localeCompare(b.name),
    );
  ...
  private findContextualSuggestions(
    ...
    if (commandName) {
      const commandHelpers = this.contextualHelpers.get(
        `command:${commandName}`,
      );
      if (commandHelpers) {
  ...
  private categorizeCommands(
    ...
    for (const command of commands) {
      ...
      if (["eval", "execute", "js"].includes(command.name)) {
        ...
        ["screenshot", "capture", "snapshot", "dom"].includes(command.name)
      ) {
        category = "Page Capture";
      } else if (
        ["log", "console", "network", "logs", "requests"].includes(
          command.name,
        ) ||
        ...
        ["click", "hover", "fill", "type", "drag", "press", "upload"].includes(
          command.name,
        )
      ) {
...
- Monitor console and network for errors`,
    ...
    this.helpTopics.set("output-formats", {
      ...
      description:
        "Understanding different output formats and processing options",
  ...
  private initializeContextualHelpers(): void {
    ...
    this.contextualHelpers.set("timeout", [
      ...
      },
      {
        error: "timeout",
        suggestion:
          "Check if the page is loading or if there are network issues",
    ...
    this.contextualHelpers.set("parse error", [
      ...
      },
      {
        error: "parse error",
        suggestion:
          "Use quotes around arguments containing spaces or special characters",
    ...
    this.contextualHelpers.set("command:screenshot", [
      {
        ...
        example:
          "mkdir -p screenshots && cdp screenshot --filename screenshots/page.png",
